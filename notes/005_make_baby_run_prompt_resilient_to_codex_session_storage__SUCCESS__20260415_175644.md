# 005_make_baby_run_prompt_resilient_to_codex_session_storage - SUCCESS

- Prompt file: `005_make_baby_run_prompt_resilient_to_codex_session_storage.md`
- Timestamp (UTC): `20260415_175644`
- Status: `SUCCESS`
...
Inspected `codex exec` behavior and confirmed the original failure was caused by session initialization trying to write through inherited state paths.

Updated `tools/codex/baby_run_prompt.py` to:

- force Codex subprocess state into a writable per-repo home at `.codex-home/`
- set `XDG_CONFIG_HOME`, `XDG_STATE_HOME`, `XDG_DATA_HOME`, and `XDG_CACHE_HOME`
- clear inherited `CODEX_THREAD_ID`
- run `codex exec` with `--ephemeral`
- detect and summarize infrastructure failures in the note
- enforce a 30 second timeout so blocked network/API access does not leave a stale `STARTED` note forever
- keep returning nonzero on actual execution failure

Verification:

- `python3 -m py_compile tools/codex/baby_run_prompt.py` passed.
- Running `./tools/codex/baby_run_prompt.py 001_smoke_test_pipeline.md` now creates a `FAILED` note instead of crashing on read-only session initialization.
- The latest smoke-test run produced `notes/001_smoke_test_pipeline__FAILED__20260415_175605.md`.
- That note records API/network failure and timeout explicitly, which is the expected degraded behavior inside this sandboxed environment.

## Notes

The direct `codex exec` path is still unable to complete inside this Codex sandbox because outbound network access is blocked here. That is distinct from the original session-storage bug and is now surfaced cleanly in the generated note.
