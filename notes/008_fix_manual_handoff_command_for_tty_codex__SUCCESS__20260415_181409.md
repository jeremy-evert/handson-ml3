...
Updated `tools/codex/baby_run_prompt.py` so manual handoff no longer prints:

```bash
codex -C "/data/git/handson-ml3" - < "/path/to/prompt.md"
```

The manual path now prints a TTY-safe workflow:

```bash
codex -C "/data/git/handson-ml3"
```

and then instructs the user to paste the contents of the prompt file into the interactive Codex session.

Verification:

* `python3 -m py_compile tools/codex/baby_run_prompt.py` passed.
* `./tools/codex/baby_run_prompt.py --manual codex_prompts/008_fix_manual_handoff_command_for_tty_codex.md` succeeded.
* The generated handoff note is `notes/008_fix_manual_handoff_command_for_tty_codex__HANDOFF__20260415_181357.md`.
* The printed instructions no longer rely on stdin redirection into interactive Codex.

## Notes

This keeps the handoff mode explicit and workable. It does not pretend to automate the interactive prompt submission; instead it launches Codex in a way that is consistent with its TTY requirement and tells the operator exactly what to do next.
