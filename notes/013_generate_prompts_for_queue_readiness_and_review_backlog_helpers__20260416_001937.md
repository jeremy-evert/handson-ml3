...
codex
I’m reading the workflow, review-gate, and recent option notes first so I can draft two bounded implementation prompts that fit the current V1 pipeline without expanding scope.
exec
...
exec
/bin/bash -lc 'rg --files codex_prompts notes | sort' in /data/git/handson-ml3
 succeeded in 0ms:
...
codex
I’ve got the workflow and option constraints. Next I’m checking the existing prompt-file style so the two new prompts match the repo’s current execution pattern and numbering.
exec
/bin/bash -lc "sed -n '1,260p' codex_prompts/013_generate_prompts_for_queue_readiness_and_review_backlog_helpers.md" in /data/git/handson-ml3
 succeeded in 0ms:
# Task: Generate the next two implementation prompts for the V1 workflow pipeline
...
exec
/bin/bash -lc "sed -n '1,260p' codex_prompts/011_build_v1_review_writeback_helper.md" in /data/git/handson-ml3
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '1,260p' codex_prompts/012_review_v1_pipeline_and_recommend_next_options.md" in /data/git/handson-ml3
 succeeded in 0ms:
...
codex
The remaining step is straightforward: I’m assigning the next prompt numbers, then I’ll write the two bounded prompt files plus the short sequencing note with a fresh UTC timestamp.
exec
...
codex
I’m writing the new files now. The prompts will target two single-purpose helpers under `tools/codex/`, with one short note explaining why readiness-checking comes before backlog listing.
apply patch
patch: completed
/data/git/handson-ml3/codex_prompts/014_build_queue_readiness_checker.md
/data/git/handson-ml3/codex_prompts/015_build_review_backlog_unreviewed_run_lister.md
/data/git/handson-ml3/notes/013_next_two_prompt_plan__20260416_002005.md
diff --git a/codex_prompts/014_build_queue_readiness_checker.md b/codex_prompts/014_build_queue_readiness_checker.md
new file mode 100644
index 0000000000000000000000000000000000000000..7138cccf9f68f460e10e4faf0f215f12ff1e5a41
--- /dev/null
+++ b/codex_prompts/014_build_queue_readiness_checker.md
@@ -0,0 +1,211 @@
...
diff --git a/codex_prompts/015_build_review_backlog_unreviewed_run_lister.md b/codex_prompts/015_build_review_backlog_unreviewed_run_lister.md
new file mode 100644
index 0000000000000000000000000000000000000000..cda4fb50dd2d72df876614ad21ea818057ae9639
--- /dev/null
+++ b/codex_prompts/015_build_review_backlog_unreviewed_run_lister.md
@@ -0,0 +1,214 @@
...
+- the existing runner and review-writeback tools remain unchanged
diff --git a/notes/013_next_two_prompt_plan__20260416_002005.md b/notes/013_next_two_prompt_plan__20260416_002005.md
new file mode 100644
index 0000000000000000000000000000000000000000..363da58d8f3f9b9defe195733bf41e038e2dd5eb
--- /dev/null
+++ b/notes/013_next_two_prompt_plan__20260416_002005.md
@@ -0,0 +1,14 @@
...
codex
The files are in place. I’m doing a quick verification pass now to confirm the numbering, scope, and “exactly two prompts plus one note” constraint are all satisfied.
exec
/bin/bash -lc "sed -n '1,260p' codex_prompts/014_build_queue_readiness_checker.md" in /data/git/handson-ml3
 succeeded in 0ms:
# Task: Build a small V1 queue-readiness checker for the next prompt
...
exec
/bin/bash -lc "sed -n '1,260p' codex_prompts/015_build_review_backlog_unreviewed_run_lister.md" in /data/git/handson-ml3
 succeeded in 0ms:
# Task: Build a small V1 review backlog and unreviewed-run lister
...
exec
/bin/bash -lc "sed -n '1,220p' notes/013_next_two_prompt_plan__20260416_002005.md" in /data/git/handson-ml3
 succeeded in 0ms:
# 013 Next Two Prompt Plan
...
from pathlib import Path
root = Path('/data/git/handson-ml3')
prompts = sorted(p.name for p in (root/'codex_prompts').glob('01[45]_*.md'))
notes = sorted(p.name for p in (root/'notes').glob('013_next_two_prompt_plan__*.md'))
...
+- any broader dashboard, queue engine, dependency system, or workflow automation layer
 ...
 succeeded in 154ms:
prompts: ['014_build_queue_readiness_checker.md', '015_build_review_backlog_unreviewed_run_lister.md']
